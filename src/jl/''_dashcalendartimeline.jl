...
- `itemDimensions` (Dict; optional): Get the dimensions for the currently rendered items. Warning: Use this prop to render custom items on a fixed timeline only. Disable scrolling, moving and resizing as it will re-render the timeline on every single event for items.
